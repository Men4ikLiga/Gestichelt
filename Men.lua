...
function isEnemyPlayer(player)
    ...
    if MenuData.SelectedTeam == "Enemy" then
        ...
        return playerTeam ~= "Class-D" and playerTeam ~= "Chaos Insurgency" and playerTeam ~= "SinglePlayer" and playerTeam ~= "None"
    end
end
...
-- FIXED ESP Update function
function ESP:UpdatePlayer(player)
    local espData = espObjects[player]
    if not espData then return end
    
    local isValid = self:IsValidTarget(player)
    local isEnemy = isEnemyPlayer(player)
    local color = getEnemyColor(player)
    
    if espData.Box then
        espData.Box.Visible = isValid and isEnemy and self.Enabled and MenuData.Running
        espData.Box.Color = color
    end
    if espData.Name then
        espData.Name.Visible = isValid and isEnemy and self.Enabled and MenuData.Running
